...
enum {
    ...
    EI_MAG3 = 3,          // File identification index.前四个字节代码 elf 文件魔数 0x7fELF
    EI_CLASS = 4,          // File class.是32位还是64位程序 ，01是32，02是64
    EI_DATA = 5,          // Data encoding.数据编码，即大小端，01 高位在前 02 低位在前。
    EI_VERSION = 6,          // File version.文件版本,固定值01
    EI_OSABI = 7,          // OS/ABI identification.OS / ABI标识。
    EI_ABIVERSION = 8,          // ABI version.ABI 版本。
    EI_PAD = 9,          // Start of padding bytes.从 9 到 15，一堆全是00的用来补全大小的数组
    EI_NIDENT = 16          // Number of bytes in e_ident.e_ident 数据的总长度
};
...
struct Elf64_Ehdr {
    ...
    Elf64_Addr    e_entry;// 可执行程序入口点地址。
    Elf64_Off     e_phoff;// Program Header Offset,程序头部表索引地址,没有则为0。
    Elf64_Off     e_shoff;// Section Header Offset,节区表索引地址,没有则为0。
    Elf64_Word    e_flags;
    Elf64_Half    e_ehsize;// ELF_Header Size,嗯..ELF头部的大小
    Elf64_Half    e_phentsize;// 程序头部表的单个表项的大小
    Elf64_Half    e_phnum;// 程序头部表的表项数
    Elf64_Half    e_shentsize;// 节区表的单个表项的大小
    Elf64_Half    e_shnum;// 节区表的表项数
